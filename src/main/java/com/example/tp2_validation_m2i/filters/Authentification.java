...
public class Authentification extends HttpFilter {
    ...
    protected void doFilter(HttpServletRequest req, HttpServletResponse res, FilterChain chain) throws IOException, ServletException {
        ...
        if(req.getRequestURI().equals(req.getContextPath() + "/login")||req.getRequestURI().equals(req.getContextPath() + "/home")) { // cette ligne autorise l'accès à l'URI /login
            chain.doFilter(req, res);
        }else{
            if(session.getAttribute("username") != null){ //vérification des bons crédentials
